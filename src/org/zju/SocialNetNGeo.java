...
//			if(contact.type == Person.HANDLER||contact.type == Person.TARGET ){
	...
	void drawHeatMap(){
		...
		fill(255,255,255);
		rectMode(CORNER);
		rect(startX-10,startY+1,20,imageHeight-3);
		rect(startX+imageWidth-352,startY+1,363,imageHeight-3);
		stroke(0);
		line(25,startY+1,width-25,startY+1);
		line(25,height-startY-2,width-25,height-startY-2);
		noStroke();
		alpha = 100;
		int yStep = 50;
		for(int i = 0;i < ctnLevelColors.size();i++){
			fill(ctnLevelColors.get(i),alpha);
			ellipse(startX+imageWidth-280,startY+250+i*yStep,30,30);
			fill(0,0,0);
			text(""+userCtnLevels.get(i)+"--"+userCtnLevels.get(i+1)+" links",startX+imageWidth-150,startY+250+i*yStep);
			alpha += 20;
		}
	}
}
